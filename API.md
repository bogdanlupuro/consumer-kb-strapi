...
- `filters[category][external_key][$eq]={category_key}` (recommended): Filter by category external_key (e.g., `cat:profile`)
  - OR `filters[category][id][$eq]={category_id}`: Filter by category ID
...
By ID:
```http
GET /api/articles?locale=en&filters[category][id][$eq]=1&populate=category
```
...
Get a specific article by ID or documentId.

**Endpoint:** `GET /articles/{id}` or `GET /articles/{documentId}`

**Query Parameters:**
- `locale` (optional): If using documentId, filter by locale. If omitted, returns default locale entry.
- `populate` (optional): Use `populate=category` to include category details
...
By ID:
```http
GET /api/articles/1?locale=en&populate=category
```

By documentId (returns the entry for the specified locale):
```http
...
2. **External Keys**: Use `external_key` for stable references across locales (e.g., `sumup.overview`, `cat:profile`).
...
5. **Relations**: Use `populate=category` to include category details in article responses and avoid extra API calls.
